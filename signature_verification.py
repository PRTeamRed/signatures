...
for writerID in train_sigs:
    ...
    score /= sc.math.factorial(len(names)-1);
    internal_variation[writerID] = sc.sqrt(score);


threshold = 800;
C = 1.8;
output = [];
for writerID in dissimilarities:
    for i in dissimilarities[writerID]:
        #if dissimilarities[writerID][i] > threshold:
        if dissimilarities[writerID][i] > internal_variation[writerID]*C:
            status = "f";
        else:
            status = "g"
        output.append(writerID+"-"+i+" "+status+"\n");

output.sort();

ground_truth_file = open("gt.txt");
